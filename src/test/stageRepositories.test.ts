...
describe("declaring a stage's sibling repositories", () => {
  it("records one per stage label and reads it back however the label is written", () => {
    const state = withStageRepository(undefined, PLAN, WORKTREE, "3D", WEB);

    assert.deepEqual(repositoriesForStage(state, PLAN, WORKTREE, "3D"), [WEB]);
    assert.deepEqual(repositoriesForStage(state, PLAN, WORKTREE, "3d"), [WEB]);
    assert.deepEqual(repositoriesForStage(state, PLAN, WORKTREE, "Stage 3D"), [WEB]);
    assert.deepEqual(repositoriesForStage(state, PLAN, WORKTREE, "3C"), [], "a declaration belongs to one stage, not to the plan");
    assert.deepEqual(repositoriesForStage(state, "another-plan", WORKTREE, "3D"), []);
  });
  ...
  it("declaring the same name again corrects the branch instead of adding a second candidate", () => {
    ...
    const first = withStageRepository(undefined, PLAN, WORKTREE, "3D", WEB);
    const corrected = withStageRepository(first, PLAN, WORKTREE, "3D", { ...WEB, branch: "feature/other" });

    assert.deepEqual(
      repositoriesForStage(corrected, PLAN, WORKTREE, "3D").map((entry) => entry.branch),
      ["feature/other"],
    );
  });

  it("keeps several distinct siblings in a stable order", () => {
    const state = withStageRepository(withStageRepository(undefined, PLAN, WORKTREE, "3D", WEB), PLAN, WORKTREE, "3D", { name: "sporely-docs", path: "/Code/sporely-docs", branch: "main" });
    assert.deepEqual(
      repositoriesForStage(state, PLAN, WORKTREE, "3D").map((entry) => entry.name),
  ...
  it("removing the last declaration leaves nothing behind", () => {
    const state = withStageRepository(undefined, PLAN, WORKTREE, "3D", WEB);
    const empty = withoutStageRepository(state, PLAN, WORKTREE, "3D", "sporely-web");

    assert.deepEqual(repositoriesForStage(empty, PLAN, WORKTREE, "3D"), []);
    assert.deepEqual(repositoriesForPlan(empty, PLAN, WORKTREE), {}, "no empty husks");
    assert.deepEqual(empty[stageRepositoryScope(PLAN, WORKTREE)], undefined);
  });

  it("removing one of several keeps the rest", () => {
    const state = withStageRepository(withStageRepository(undefined, PLAN, WORKTREE, "3D", WEB), PLAN, WORKTREE, "3D", { name: "sporely-docs", path: "/Code/sporely-docs", branch: "main" });
    assert.deepEqual(
      repositoriesForStage(withoutStageRepository(state, PLAN, WORKTREE, "3D", "sporely-docs"), PLAN, WORKTREE, "3D").map((entry) => entry.name),
      ["sporely-web"],
    );
  });
  ...
  it("drops junk from workspace state rather than throwing on it", () => {
    const corrupt = {
      [stageRepositoryScope(PLAN, WORKTREE)]: {
        "3D": [WEB, { name: "no-branch", path: "/x" }, { name: "", path: "/y", branch: "main" }, "not an object", null],
        "3C": [],
      },
    } as unknown as StageRepositories;

    assert.deepEqual(repositoriesForStage(corrupt, PLAN, WORKTREE, "3D"), [WEB]);
    assert.deepEqual(repositoriesForPlan(corrupt, PLAN, WORKTREE), { "3D": [WEB] });
    assert.deepEqual(repositoriesForPlan(undefined, PLAN, WORKTREE), {});
    assert.deepEqual(repositoriesForPlan({ [stageRepositoryScope(PLAN, WORKTREE)]: "nonsense" } as unknown as StageRepositories, PLAN, WORKTREE), {});
  });
