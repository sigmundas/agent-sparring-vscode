...
async function buildFixture(): Promise<{ root: string; workspaceFile: string }> {
  ...
  await fs.writeFile(
    ...
    [
      ...
      '# its Agents line. It is deliberately NOT written to fake-calls.log:',
      ...
      '  model=null; [ -f "$cfg" ] && grep -q \'^model\' "$cfg" && model=\'"opus"\'',
      '  printf \'{"config_path": "%s", "config_exists": %s, "project": "fixture", "error": null,\' "$cfg" "$exists"',
      '  printf \' "stage": {"role": "stage", "provider": "claude-cli", "provider_display_name": "Claude", "provider_source": "engine-default", "model": %s, "model_source": "provider-default", "effort": null, "effort_source": "provider-default", "effort_supported": true},\' "$model"',
      '  printf \' "sparring": {"role": "sparring", "provider": "codex-cli", "provider_display_name": "Codex", "provider_source": "engine-default", "model": null, "model_source": "provider-default", "effort": null, "effort_source": "provider-default", "effort_supported": true}}\\n\'',
      '  exit 0',
      'fi',
